jQuery(document).ready(function($) {
	...
	$(document).dblclick(function(event) {
		...
		if( $('#mk_builder_editor_metabox.open').length && !$(event.target).closest("#mk_builder_editor_metabox.open").length) {
			...
			if( $(event.target).closest(".mk_editor_screen_overlay.open").length )
			{
				if( tinyMCE.activeEditor == null || tinyMCE.activeEditor != null && !tinyMCE.activeEditor.isDirty() )
				{
					close_mk_editor();
				}
			}
		...
		if( $('.mk_editor_sectie.open').length && !$(event.target).closest(".mk_editor_sectie.open").length) {
			...
			if( !$(event.target).closest(".mk_editor_screen_overlay_editor.open").length )
			{
		...
		//Template window laden opslaan
		if( $('#mk-builder-opslaan-laden.open').length && !$(event.target).closest("#mk-builder-opslaan-laden.open").length) {
		...
		{
			//Page Kopieren
			if( $('.mk_sectie_opslaan_window').length && !$(event.target).closest(".mk_sectie_opslaan_window").length) {
		...
		//New rij
		if( $('.use_builder_box.open').length && !$(event.target).closest(".use_builder_box").length) {
	...
	$('.allepaginas_menu ul li').click(function()
	{
		var id = $(this).attr('id');
		$('.paginatype').addClass('hide');
		$('.paginatype*[id="'+id+'"]').removeClass('hide');

		$('.allepaginas_menu ul li').removeClass('active');
...
function Create_new_knop_Module(rij_id, kolom_id, module_class, module_slot, module_link_id, module_link_titel, module_link_url, module_target, module_extern, module_bestand)
{
	MK_Append_New_Module_knop( rij_id, countmod, kolom_id, module_class, module_slot, module_link_id, module_link_titel, module_link_url, module_target, module_extern,  module_bestand );

	countmod++;
}

//Create_new_knop_Module(countrij, kolom_ID, module_class, module_slot, module_link_id, mkmodule[m], module_link_url, module_target, module_extern);
function Create_new_bestand_Module(rij_id, kolom_id, module_class, module_slot, module_link_titel, module_link_url, module_link_id)
{
	MK_Append_New_Module_bestand( rij_id, countmod, kolom_id, module_class, module_slot, module_link_titel, module_link_url, module_link_id);
...
function Create_new_video_Module(rij_id, kolom_id, url, optie, module_class, module_slot, module_target, module_poster)
{
	MK_Append_New_Module_Video( rij_id, countmod, kolom_id, url, optie, module_class, module_slot, module_target, module_poster );
...
function AddModuleAfbeelding(att, countrij, kolom) { jQuery(function ($) {
...
function AddModuleGalerij(att, countrij, kolom) { jQuery(function ($) {
...
function AddModuleCode(att, countrij, kolom) { jQuery(function ($) {
...
function AddModuleKnop(att, countrij, kolom) { jQuery(function ($) {
...
function AddModuleBestand(att, countrij, kolom) { jQuery(function ($) {

	MK_Append_New_Module_bestand(countrij , countmod, kolom, "", "", "", "", "");
...
function AddModuleLijst(att, countrij, kolom) { jQuery(function ($) {
...
function AddModuleVideo(att, countrij, kolom) { jQuery(function ($) {
...
function delete_no(id)  { jQuery(function ($) {
...
//  Delete sectie
var delete_sectie = -1;
function deleteSectie(id)  { jQuery(function ($) {
	if( delete_sectie == id)
	{
...
function delete_sectie_yes(id)  { jQuery(function ($) {
...
function prullenbaktoggle() { jQuery(function ($) { 

	$('.builderprullenbak').toggleClass('open'); 
...
//  Delete Rij
var delete_rij = -1;
function deleteRij(id)  { jQuery(function ($) {
...
function delete_rij_yes(id) { jQuery(function ($) {
...
//  Delete Module
delete_module = -1
function deleteModule(id)  { jQuery(function ($) {
	if( delete_module == id)
	{
...
function delete_module_yes(id)  { jQuery(function ($) {
...
function restoreModule(id) { jQuery(function ($) {

}); }


//Official builder
function MK_Append_New_Sectie(sectie_id, sectie_cssid, sectie_class, sectie_bgcolor, sectie_bgimg, sectie_slot, sectie_setting, sectie_type )  { jQuery(function ($) {
...
//  sectieafter
function sectieafter(id) { jQuery(function ($) {
...
function MK_Append_New_Sectie_After(id, sectie_id, sectie_cssid, sectie_class, sectie_bgcolor, sectie_bgimg, sectie_slot, sectie_setting, sectie_type) { jQuery(function ($) {
...
function MK_Append_New_Rij(sectie_id, rij_id, rij_class)  { jQuery(function ($) {
...
function MK_Append_New_Rij_2_koloms(sectie_id, rij_id, rij_class)  { jQuery(function ($) {
...
function MK_Append_New_Rij_3_koloms(sectie_id, rij_id, rij_class)  { jQuery(function ($) {
...
function MK_Append_New_Rij_4_koloms(sectie_id, rij_id, rij_class)  { jQuery(function ($) {
...
function MK_Append_New_Module_tekst(rij_id, mod_id, kolom_id, text, module_class, module_slot)  { jQuery(function ($) {
...
function MK_Append_New_Module_Titel(rij_id, mod_id, kolom_id, text, module_class, tag_h, module_slot)  { jQuery(function ($) {
...
function MK_Append_New_Module_Afbeelding(rij_id, mod_id, kolom_id, module_class, module_img, module_slot, module_size, module_url)  { jQuery(function ($) {
...
function MK_Append_New_Module_galerij(rij_id, mod_id, kolom_id, module_class, module_galerij, module_slot, module_view, module_size, module_target)  { jQuery(function ($) {
...
function MK_Append_New_Module_Code(rij_id, mod_id, kolom_id, text, module_class, module_slot)  { jQuery(function ($) {
...
function MK_Append_New_Module_knop(rij_id, mod_id, kolom_id, module_class, module_slot, module_link_id, module_link_titel, module_link_url, module_target, module_extern, module_bestand)  { jQuery(function ($) {
	...
	$("#rij_" + rij_id + " .kolom.kolom_" + kolom_id +" .newrij_inner" ).append(
		...
		'<li id="mod_id_'+mod_id+'" class="module module_knop" type="knop">'+
			...
			'<div class="mod_titel">'+
				...
				'<div class="mod_gegevens" module_class="'+module_class+'" module_link_url="'+module_link_url+'" module_link_titel="'+module_link_titel+'"  module_link_id="'+module_link_id+'" module_slot="'+module_slot+'" module_target="'+module_target+'" module_extern="'+module_extern+'" module_bestand="'+module_bestand+'"></div>'+
...
// module_target="'+module_target+'" module_extern="'+module_extern+'" module_bestand="'+module_bestand+'"

function MK_Append_New_Module_bestand(rij_id, mod_id, kolom_id, module_class, module_slot, module_link_titel, module_link_url, module_link_id)  { jQuery(function ($) {
...
//End MK_Append_New_Module_bestand
...
function MK_Append_New_Module_lijst(rij_id, mod_id, kolom_id, text, module_class, module_slot)  { jQuery(function ($) {
...
//Video module
function MK_Append_New_Module_Video(rij_id, mod_id, kolom_id, url, optie, module_class, module_slot, module_target, module_poster)  { jQuery(function ($) {
...
function getallimages() { jQuery(function ($) {
		 ...
		 // console.log(array_galerij.length);
		    ...
		    for(var i = 0; array_galerij.length; i++)
		    {
		    	...
		    	if(array_galerij[i] != "" && array_galerij[i] != null)
		    	{
					...
					if(array_galerij_type[i] == "videofile" && temp_image_json != null)
					{
						//console.log('item video : ' + temp_image_json.toJSON() ) ; // + temp_image_json.get('url') );
						//console.log('item video : ' + temp_image_json.get('id') ) ;
						//console.log('item video : ' + temp_image_json.get('title') ) ;

						$(array_galerij_location[i]).find('.titel').html(  temp_image_json.get('title') );
						$(array_galerij_location[i]).find('.url').html(  temp_image_json.get('url') );
					}
					if(array_galerij_type[i] == "bestand" && temp_image_json != null)
					{
						//console.log('item video : ' + temp_image_json.toJSON() ) ; // + temp_image_json.get('url') );
						//console.log('item video : ' + temp_image_json.get('id') ) ;
						//console.log('item video : ' + temp_image_json.get('title') ) ;

						//$(array_galerij_location[i]).attr('.titel',  temp_image_json.get('title') );
						$(array_galerij_location[i]).find('.url').html(  temp_image_json.get('url') );

						//$(array_galerij_location[i]).html( temp_image_json.get('title') + ' : ' + temp_image_json.get('url') );
					}
					else if(temp_image_json.get('sizes') != null) { //console.log('falback'); return; }
						...
						{
							$(array_galerij_location[i]).append( '<li class="image" img_thumb="'+thumb+'" img_id="'+array_galerij[i]+'" img_url="'+img_url+'"><div class="remove" onclick="galerijremoveimage(this)"></div><div class="crop" onclick="galerijcropimage('+array_galerij[i]+')">Snij afbeelding bij</div><img src="'+ thumb +'"/></li>' );
						}
						...
						else if(array_galerij_type[i] == "videoposter")
						{
							$(array_galerij_location[i]).attr('poster', temp_image_json.get('url') );

							//console.log('sdojfodjfodjsofdks : ' +   temp_image_json.get('url'));
						}
...
function addmedia(id) { jQuery(function ($) {
...
function mk_editor_addmedia() { jQuery(function ($) {
	//afbeeldingnopen = 1;
...
function mk_editor_achtergrond_media() { jQuery(function ($) {
	//windowopen = 1;
...
//add video
function mk_editor_addvideo() { jQuery(function ($) {
	...
	.on('select', function(){
		...
		$('#mk_editor_video_url').html(uploaded_image.toJSON().url);
		$('#mk_editor_video_naam').html(uploaded_image.toJSON().title);

		$('#mk_editor_video_hold img').attr('src', image_url);

		$('#mk_editor_video_id').html(image_id);
		$('.mk_editor_module_video input.video').val(image_id);
	});

}); }

//add video poster
function mk_editor_addvideo_poster() { jQuery(function ($) {
	...
	.on('select', function(){
		...
		$('#mk_editor_video_url_poster img').attr('src', image_url);
		$('#mk_editor_video_naam_poster').html(uploaded_image.toJSON().title);
		$('#mk_editor_video_id_poster').html( image_id );

		//$('#mk_editor_video_poster_hold img').attr('src', image_url);
...
//add bestand
function mk_editor_addbestand() { jQuery(function ($) {
	...
	.on('select', function(){
		...
		$('#mk_editor_bestand_url').html( image_url);
		$('#mk_editor_bestand_naam').html(uploaded_image.toJSON().title);
		$('#mk_editor_bestand_id').html( image_id );

		$('.mk_editor_module_bestand .bestandurl').val( image_url );

		if( $('.mk_editor_module_bestand .bestandtitel').val() == "" ) 
		{
			$('.mk_editor_module_bestand .bestandtitel').val( uploaded_image.toJSON().title );
		}
	});
...
function addgalerij() { jQuery(function ($) {
        ...
        .on('select', function(e){
           ...
           for (i = 0; i < attachments.length; ++i) {
               ...
               // console.log(  );
                ...
                $('#galerij_wrap').append( '<li class="image" img_thumb="'+thumb+'" img_id="'+attachments[i].attributes.id+'" img_url="'+attachments[i].attributes.url+'"><div class="remove" onclick="galerijremoveimage(this)"></div><div class="crop" onclick="galerijcropimage('+attachments[i].attributes.id+')">Snij afbeelding bij</div><img src="'+ thumb +'"/></li>' );
...
function galerijremoveimage(e) { jQuery(function ($) {

	$(e).parent().remove();

}); }

function galerijsameheight() { jQuery(function ($) {
...
function addMultipleMedia(id) {jQuery(function ($) {
...
function tinymce22() { jQuery(function ($) {
...
function AddModule(att, countrij, kolom) { jQuery(function ($) {
...
function mk_builder_new_module_close()  { jQuery(function ($) {
...
function Append_add_new_Module(att, countrij, kolom) { jQuery(function ($) {
	...
	$( "body" ).append( 
		...
		'<div class="mk_builder_new_module">'+
			...
			'<div class="mk_builder_new_module_container">'+
				...
				'<div class="addmodule bestand" name="Modules_code" id="Modules" onclick="AddModuleBestand('+ att +', '+ countrij +', '+kolom+')">Bestand</div>'+
...
function module_kopieren(module_id) { jQuery(function ($) {
...
// New RIJ
*/
function addrij(sectie_id) { jQuery(function ($) {
...
//addrij
function mk_builder_new_rij_close()  { jQuery(function ($) {
...
//mk_builder_new_rij_close
function Append_add_new_rij(sectie_id)  { jQuery(function ($) {
...
function mkbuilder_standaard_editor() { jQuery(function ($) {

	use_stardaard_builder();


	var post_id = $('.mkbuilder_gegevens').attr('post_id');
	var domain = $('.mkbuilder_gegevens').attr('domain') + '/wp-admin/admin-ajax.php';
	...
	jQuery.ajax({
		...
		url: domain,
		data: {
			action: 'mk_builder_use',
			post_id: post_id,
			builder: 'standaard',
		},
		beforeSend: function() {
			//$('.mk_button.meeritems').css('opacity', '0.5');
		},
		success: function (output) {
			//console.log('succes!! updated: ' + output); 
		},
...
function mkbuilder_use_mk_builder() { jQuery(function ($) {

	use_mk_builder_after_standaard();
